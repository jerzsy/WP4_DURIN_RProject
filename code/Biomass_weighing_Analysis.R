...
#Boxplot 
## Boxplot of leaf biomass for VV species, by site and habitat - individuals only
biomass_vv_weighed %>%
  filter(IndividualID %in% c("VV_1", "VV_2", "VV_3")) %>% # only individuals VV_1, VV_2 and VV_3
  ggplot(aes(x = siteID, y = biomass_alive_leaves_total, fill = habitat)) +
  geom_boxplot() +
  labs(
    title = "Leaf Biomass for Vaccinium vitis-idaea by Site and Habitat",
    x = "Site",
    y = "Leaf Biomass (g)"
  ) +
  theme_minimal()

## Boxplot of stem biomass for VV species, by site and habitat - individuals only
biomass_vv_weighed %>%
  filter(IndividualID %in% c("VV_1", "VV_2", "VV_3")) %>% # only individuals VV_1, VV_2 and VV_3
  ggplot(aes(x = siteID, y = biomass_brown_stem_total, fill = habitat)) +
  geom_boxplot() +
  labs(
    title = "Stem Biomass for Vaccinium vitis-idaea by Site and Habitat",
    x = "Site",
    y = "Stem Biomass (g)"
  ) +
  theme_minimal()
